...
typedef enum {
  INTERRUPT_PIN_ACTIVE_HIGH =
      0, /*!< The mpu6050 sets its INT pin HIGH on interrupt */
  INTERRUPT_PIN_ACTIVE_LOW =
      1 /*!< The mpu6050 sets its INT pin LOW on interrupt */
} mpu6050_int_pin_active_level_t;

typedef enum {
  INTERRUPT_PIN_PUSH_PULL =
      0, /*!< The mpu6050 configures its INT pin as push-pull */
  INTERRUPT_PIN_OPEN_DRAIN =
      1 /*!< The mpu6050 configures its INT pin as open drain*/
} mpu6050_int_pin_mode_t;

typedef enum {
  INTERRUPT_LATCH_50US =
      0, /*!< The mpu6050 produces a 50 microsecond pulse on interrupt */
  INTERRUPT_LATCH_UNTIL_CLEARED =
      1 /*!< The mpu6050 latches its INT pin to its active level, until
           interrupt is cleared */
} mpu6050_int_latch_t;

typedef enum {
  INTERRUPT_CLEAR_ON_ANY_READ =
      0, /*!< INT_STATUS register bits are cleared on any register read */
...
typedef struct {
  ...
  mpu6050_int_pin_active_level_t
      active_level; /*!< Active level of mpu6050 INT pin         */
  mpu6050_int_pin_mode_t
      pin_mode; /*!< Push-pull or open drain mode for INT pin*/
  mpu6050_int_latch_t
      interrupt_latch; /*!< The interrupt pulse behavior of INT pin */
  mpu6050_int_clear_t
      interrupt_clear_behavior; /*!< Interrupt status clear behavior         */
} mpu6050_int_config_t;

extern const uint8_t MPU6050_DATA_RDY_INT_BIT; /*!< DATA READY interrupt bit */
extern const uint8_t
    MPU6050_I2C_MASTER_INT_BIT; /*!< I2C MASTER interrupt bit               */
extern const uint8_t
    MPU6050_FIFO_OVERFLOW_INT_BIT; /*!< FIFO Overflow interrupt bit */
extern const uint8_t
    MPU6050_MOT_DETECT_INT_BIT; /*!< MOTION DETECTION interrupt bit         */
extern const uint8_t
    MPU6050_ALL_INTERRUPTS; /*!< All interrupts supported by mpu6050    */
 ...
 *     - Others Success
 */
mpu6050_handle_t mpu6050_create(i2c_master_dev_handle_t *const dev_handle);
 ...
 *
 * This function does not disable interrupts not set in interrupt_sources. To
 * disable specific mpu6050 interrupts, use mpu6050_disable_interrupts().
 *
 ...
 *
 * This function does not enable interrupts not set in interrupt_sources. To
 * enable specific mpu6050 interrupts, use mpu6050_enable_interrupts().
 *
 ...
 *
 * @param sensor object handle of mpu6050
 * @param out_intr_status[out] bit mask that is assigned a value representing
 * the interrupts triggered by the mpu6050
 *
 ...
 *
 * The behavior of the INT_STATUS register of the mpu6050 may change depending
 * on the value of mpu6050_int_clear_t used on interrupt configuration.
 *
 * @return
 *      - ESP_OK Success
 *      - ESP_ERR_INVALID_ARG A parameter is NULL or not valid
 ...
 *
 * @param interrupt_status mpu6050 interrupt status, obtained by invoking
...
/**
 * @brief Determine if the last mpu6050 interrupt was triggered by a fifo
 * overflow.
 *
 * @param interrupt_status mpu6050 interrupt status, obtained by invoking
 * mpu6050_get_interrupt_status()
...
esp_err_t mpu6050_complimentory_filter(
    mpu6050_handle_t sensor, const mpu6050_acce_value_t *const acce_value,
